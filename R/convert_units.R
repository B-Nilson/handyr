...
convert_units <- function(x, from = NULL, to, keep_units = NULL, ...) {
  rlang::check_installed("units")
  # handle inputs
  stopifnot(is.numeric(x), is.character(from) | is.null(from), is.character(to))
  stopifnot(length(from) == 1 | is.null(from), length(to) == 1)
