...
#'
#' @param x A numeric or date/POSIXct vector.
#' @param range A numeric or date/POSIXct vector of length two to clamp `x` to.
...
clamp <- function(x, range = c(NA, NA)) {
  # Handle inputs
  stopifnot(
    is.numeric(x) || lubridate::is.Date(x) || lubridate::is.POSIXct(x)
  )
  stopifnot(
    length(range) == 2,
    is.numeric(range) ||
